...
def train(args,train_ds,val_ds):
    ...
    if args.verbose : 
        ...
        if p.requires_grad)
        my_param = n_params - n_params_embeddings
        print(f"Number of model trainable parameters : {my_param}")
    ...
    return all_metrics, checkpoint_path,my_param


########################################################################################
########################################################################################

def train_m_models(args, M:int=None, train_ds = None, val_ds=None, seeds:list=None):
    ...
    for seed, m in zip(seeds, range(M)):
        ...
        all_metrics, checkpoint_path,my_param = train(args,train_ds,val_ds) # Train the model
