...
class LayerNorm(nn.Module):
    ...
    def forward(self, inputs):
        ...
        input_var = torch.mean(torch.pow(inputs-input_mean,2),dim=-1,keepdim=True)

        normalized_input = (((inputs-input_mean)/torch.sqrt((input_var+self.eps)))*self.weight) + self.bias
...
class MultiHeadedAttention(nn.Module):
    ...
    def get_attention_weights(self, queries, keys):
        ...
        weights_logits = torch.matmul(queries,keys.transpose(-2,-1))/math.sqrt(self.head_size)
        ...
        weights_logits =  weights_logits + mask

        attention_weights = torch.softmax(weights_logits,dim=-1)
    ...
    def apply_attention(self, queries, keys, values):
        ...
        merged_values = self.merge_heads(attended_values)

        return merged_values,attention_weights
    ...
    def split_heads(self, tensor):
        ...
        output_tensor = tensor.reshape(batch_size,sequence_length,self.num_heads,-1)
    ...
    def merge_heads(self, tensor):
        ...
        output_tensor = tensor.reshape(batch_size,sequence_length,-1)
    ...
    def forward(self,  queries: Tensor, keys: Tensor, values: Tensor):
        ...
        context_attention,attn_weights = self.apply_attention(Q,K,V)
        attn_weights = attn_weights.clone().detach()
        OUT = self.W_O(context_attention)
...
class GPTEmbedding(nn.Module):
    ...
    def create_sinusoidal_embeddings(self, n_positions: int, dimension: int) -> torch.FloatTensor:
        ...
        """
        PE = torch.empty(n_positions,dimension)
        for position in range(n_positions):
            for i in range(int(dimension/2)):
                PE[position,2*i] = math.sin(position/(10000**(2*i/dimension)))
                PE[position,2*i+1] = math.cos(position/(10000**(2*i/dimension)))
    ...
    def forward(self, tokens: Tensor) -> Tensor:
        ...
        positional_embedding_indexed = self.position_encoding[:sequence_length,:].unsqueeze(0).expand(batch_size,sequence_length,-1) #copy over batch size
        
        final_embeddings = token_embeddings + positional_embedding_indexed
...
class GPT(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tuple[Tensor, Union[Tensor, None], Union[Tensor, None]]:
        ...
        a, (hidden_states, attentions) = self.decoder(embeddings)
