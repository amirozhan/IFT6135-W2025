...
class LSTMCell(nn.Module):
    ...
    def forward(self, x, hx):
        ...
        candidate_cell = torch.tanh(self.candidate_cell(combined))

        candidate_cell_update = forget_gate*c + input_gate*candidate_cell
        hidden_state_update = output_gate*torch.tanh(candidate_cell_update)


        return hidden_state_update,candidate_cell_update
########################################################################################
########################################################################################
...
class LSTM(nn.Module):
    ...
    def forward(self, x, hx=None):
        ...
        for layer_idx, cell in enumerate(self.layers):
            h_t = h0[layer_idx]  # (batch_size, hidden_size)x
            ...
            for t in range(seq_len):
                ...
                #else:
                  #  h_t,c_t = cell(h0[layer_idx-1] ,(h_t,c_t))
...
class LSTMLM(nn.Module):
    ...
    def forward(self, x, hidden_states=None):
        ...
        out,(h_final,c_final) = self.lstm(out,hidden_states)
